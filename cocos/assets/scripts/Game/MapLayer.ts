...
export default class MapLayer extends Component {
  ...
  revertFence() {
    ...
    for (let i = 0; i < 7; i++) {
      this._revertAction.delay(0.3).call(() => this.changeFence((i % 2) + 1));
    }
  ...
  changeFence(fenceType) {
    ...
    for (let { line, col } of fencePos) {
      ...
      if (block) {
        ...
        block.getComponent(Sprite).spriteFrame =
          this.fenceBlockDatas[fenceType].blockFrame;
      }
    }
  }

  spawnNewEnemy() {
    if (this.enemies.children.length >= 100) return;

    if (this._remainEnemiesCount <= 0) return;
  ...
  createEnemy(pos: Vec2) {
    ...
    // 更新信息区域
    find("/Canvas/GameLayer/Informations")
