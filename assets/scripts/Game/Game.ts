...
export default class Game extends Component {
  ...
  gameOver() {
    ...
    gameOverNode.setPosition(
      0,
      -visableSize.height / 2 -
        gameOverNode.getComponent(UITransformComponent).height / 2
    );
  ...
  runAction(tween) {
    if (!tween || !tween._target) return tween;
  ...
  showAnimation() {
    ...
    blackUp.setPosition(
      0,
      visableSize.height / 2 + blackUp.getComponent(UITransform).height / 2
    );

    let blackDown = instantiate(this.black);
    blackDown.getComponent(UITransform).width = visableSize.width;
    blackDown.setPosition(
      0,
      -(visableSize.height / 2 + blackDown.getComponent(UITransform).height / 2)
    );
