...
export default class PlayerTank extends BaseTank {
  ...
  control(dir: Dir) {
    ...
    if (!this.autoMoving) this._audioMng.playAudio("player_move", true);
  ...
  shoot() {
    ...
    this.mapLayer.createBullet(
      this.dir,
      this.node.position,
      Math.min(5, (this.level + 1) * 2),
      this,
      this.level
    );
  }
  ...
  disBlood() {
    ...
    // 更新剩余生命值
    if (this.life > 0) this._ui.updatePlayerBlood(this.life - 1);
  }
  ...
  gameOver() {
    ...
    gameOverNode.setPosition(
      -visableSize.width / 2 - gameOverNode.getComponent(UITransform).width / 2,
      -94
    );
  ...
  reset() {
    ...
    this.getComponent(Animation).crossFade("star", 0.5);
    this.canMove = false;
    this.blood = 1;
    this._isInvincible = true;
    this._setDir(Dir.UP);
  ...
  onLoad() {
    ...
    this.life = 60;
    this.isEnemy = false;
    this._ui = find("/Canvas/GameLayer/Informations").getComponent(
      UpdateInformations
    );
  ...
  changeLevel(player, value = 1) {
    ...
    if (
      value > 0 &&
      this.level >= Globals.TB_DOUBLE_POWER &&
  ...
  _addMemDir(dir: Dir) {
    if (!this._memDirs.includes(dir)) this._memDirs.push(dir);
  }
  ...
  _setDir(dir: Dir) {
    ...
    // 如果产生碰撞，则回到之前的位置
    if (
      this._isCollisionWithMap() ||
      this._isCollisionWithBlock() ||
